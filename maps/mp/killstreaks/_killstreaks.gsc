...
// Blocklist for the "enemy team can't get certain killstreaks" restriction -
// "enemy team" here means anyone isRestrictedPlayer() (_toolbridge.gsc)
// flags, normally anyone who ISN'T part of the vertu tool's party
// (isToolPartyMember), matching how the rest of the tool already
// distinguishes the party's side from everyone else (see _toolbridge.gsc /
// enforcePartyIsPlantTeam) - or, with the Test tab's "Apply Restrictions To
// Host" checkbox on, EVERYONE. Checked from both checkKillstreakReward() (so
// the "streak earned" splash never fires for a streak that won't be given)
// and giveKillstreak() (the single choke point every delivery path routes
// through - earned streaks, the AC-130 second gunner seat, and airdrop/care
// package crate pickups - so this can't be bypassed via those side doors).
isKillstreakBlockedForEnemyTeam( streakName )
{
	if ( !maps\mp\gametypes\_toolbridge::isRestrictedPlayer( self ) )
		return false;

	blockedStreaks = [];
	blockedStreaks[ blockedStreaks.size ] = "predator_missile";
	blockedStreaks[ blockedStreaks.size ] = "harrier_airstrike";
	blockedStreaks[ blockedStreaks.size ] = "precision_airstrike";
	blockedStreaks[ blockedStreaks.size ] = "ac130";
	blockedStreaks[ blockedStreaks.size ] = "helicopter";
	blockedStreaks[ blockedStreaks.size ] = "helicopter_flares";
	blockedStreaks[ blockedStreaks.size ] = "helicopter_minigun";
	blockedStreaks[ blockedStreaks.size ] = "helicopter_mk19";

	foreach ( blockedStreak in blockedStreaks )
	{
		if ( streakName == blockedStreak )
			return true;
	}
...
checkKillstreakReward( streakCount )
{
	...
	foreach ( streakVal, streakName in self.killStreaks )
	{
		...
		if ( self isKillstreakBlockedForEnemyTeam( useStreakName ) )
			continue;
...
giveKillstreak( streakName, isEarned, awardXp, owner )
{
	...
	// safety net for delivery paths that don't go through
	// checkKillstreakReward() at all (AC-130 second gunner seat, airdrop /
	// care package crate pickups) - see isKillstreakBlockedForEnemyTeam().
	if ( self isKillstreakBlockedForEnemyTeam( streakName ) )
		return;
