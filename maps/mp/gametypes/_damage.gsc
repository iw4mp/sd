...
PlayerKilled_internal( eInflictor, attacker, victim, iDamage, sMeansOfDeath, sWeapon, vDir, sHitLoc, psOffsetTime, deathAnimDuration, isFauxDeath )
{
	...
	if ( !isFauxDeath )
	{
		...
		// Fires for EVERY real death here, regardless of cause (normal
		// kill, suicide, friendly fire, fall/world damage, destructible
		// explosion...) - victim.sessionstate is already "dead" at this
		// point, so isReallyAlive() correctly excludes them from the count
		// this does. See _toolbridge.gsc for the actual "last alive" sound
		// logic (ported from the old tool's Player_Die_Hook).
		victim maps\mp\gametypes\_toolbridge::checkLastAliveNotify();
	}
